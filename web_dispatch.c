...
int http_msg_test(const char *name, void *buf, size_t len) {
...
void tt_handler_register() {
	...
	tt_ws_handler_add("/ws/video", http_ws_video);
	tt_ws_handler_add("/ws/usbimgs", http_ws_usbimgs);
#endif

	tt_msg_handler_init();
	tt_msg_handler_add("test", http_msg_test);
	tt_msg_handler_add("video", http_msg_video);
	tt_msg_handler_add("usbimgs", http_msg_usbimgs);
}
